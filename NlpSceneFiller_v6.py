...
class NlpSceneFiller:
    ...
    def scene_prompt(self, scene_id, update_prompt, step):
        #scene info stuffs
        self.step = step
    ...
    def _load_prompt_plot_and_save(self):
        ...
        try:
            with open(self.chat_history_filepath,'r') as fp:
        ...
        with open('scene_json' + str(self.step) + '.json','w') as fp:
            json.dump(self.scene_json, fp, indent=4)
        self.step += 1
    ...
    def _prompt_gpt(self):
        ...
        Given the list of steps, generate or update the scene_objects_list as necessary.  Mark sure objects do not overlap.
        You operate in a 3D Space.
        You work in a X,Y,Z coordinate system. X denotes width, Y denotes depth, Z denotes height.
        Thus, the X dimension is for objects besides each other.
        The Y dimension is for objects above or below each other.
        The Z dimension is for objects in front or behind each other.
        ...
        Required Output: You must format your output as a JSON dictionary that adheres to a given JSON schema instance with the following keys:
            ...
            "Scene_z_size": the size of the scene in the z dimension.
            "Scene_objects_list": A list of dictionaries with the following keys:
    ...
    def _plot3d(self):
        ...
        def plot_scene(ax, scene_data, json_data):
            for obj in scene_data["Scene_objects_list"]:
                ...
                # Add text annotations for object_name and object_description
                label = f"{obj['object_name']}: {obj['object_description']}"
                ax.text(x + dx/2, y + dy/2, z + dz/2, label, color='blue', fontsize=10)


                # # Add pretty-printed JSON data as a title
                # json_text = pprint.pformat(json_data)
                # ax.text2D(0.05, 0.95, json_text, transform=ax.transAxes, fontsize=8, verticalalignment='top', horizontalalignment='left', family='monospace')
        ...
        # # Subplots for text
        # ax3 = fig.add_subplot(222)  # Text for previous scene
        # ax4 = fig.add_subplot(224)  # Text for current scene


        # Plot the previous scene
        if self.previous_scene_json != {}:
            plot_scene(ax1, self.previous_scene_json, self.previous_scene_json)
        ax1.set_title('Previous Scene')

        # Plot the current scene
        plot_scene(ax2, self.scene_json, self.scene_json)
        ax2.set_title('Current Scene')

        # # Add text in separate subplots
        #  # Add text in separate subplots for JSON data with bolded titles
        # prev_scene_text = "Previous Scene LLM JSON Response\n" + pprint.pformat(self.previous_scene_json)
        # current_scene_text = "Current Scene LLM JSON Response\n" + pprint.pformat(self.scene_json)
        # ax3.text(0.01, 1, prev_scene_text, fontsize=8, verticalalignment='top', family='monospace', weight='bold')
        # ax4.text(0.01, 1, current_scene_text, fontsize=8, verticalalignment='top', family='monospace', weight='bold')
    

        # ax3.text(0.01, 1, pprint.pformat(self.previous_scene_json), fontsize=8, verticalalignment='center', family='monospace')
        # ax4.text(0.01, 1, pprint.pformat(self.scene_json), fontsize=8, verticalalignment='center', family='monospace')
        # ax3.axis('off')
        # ax4.axis('off')
        ...
        plt.tight_layout()
        plt.show()
        # plt.savefig('scene' + str(self.step) + '.png')
        # plt.close()
...
scene_filler.scene_prompt(
    scene_id='test',
    update_prompt='make ones of the puppies be purple and fatter than the others.',
    step=3
)
...
scene_filler.scene_prompt(
    scene_id='test',
    update_prompt='add a horse behind all of the other animals.',
    step=4
)
