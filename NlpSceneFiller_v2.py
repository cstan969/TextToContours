...
class NlpSceneFiller:
    ...
    def _prompt_gpt(self):
        ...
        """
        prompt_from_template = PromptTemplate(template=template, input_variables=["previous_scene_json","update_prompt", "previous_scene_description"])
        llm_chain = LLMChain(prompt=prompt_from_template,llm=self.llm, verbose=True)
        response = llm_chain.run(previous_scene_json=str(self.previous_scene_json), update_prompt=self.update_prompt, previous_scene_description=self.previous_scene_description)
        print(response)
        try:
