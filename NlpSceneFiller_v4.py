...
class NlpSceneFiller:
    ...
    def _plot3d(self):
        ...
        fig = plt.figure(figsize=(30, 10))  # Adjust the size as needed
        ...
        ax3 = fig.add_subplot(133)  # Subplot for text


        # Plot the previous scene
        if self.previous_scene_json != {}:
            plot_scene(ax1, self.previous_scene_json)
            ax1.set_title('Previous Scene')

        # Plot the current scene
        plot_scene(ax2, self.scene_json)
        ax2.set_title('Current Scene')

        # Display the JSON data in the third subplot
        json_text = "Previous Scene JSON:\n" + pprint.pformat(self.previous_scene_json) + "\n\n" + \
                    "Current Scene JSON:\n" + pprint.pformat(self.scene_json)
        ax3.text(0.01, 0.5, json_text, fontsize=8, verticalalignment='center', horizontalalignment='left', multialignment='left', family='monospace')
