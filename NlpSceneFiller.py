...
class NlpSceneFiller:
    ...
    def _prompt_gpt(self):
        template = """
        '''''
        You are an area generator expert
        Given an area of a certain size, you can generate a list of items that are appropriate to that area, in the right place.
        ...
        You receive from the user the objects that they want in the area, or alternatively, the changes they want applied to the area in the form of natural language.  Keep in mind, objects should be placed in the area to create the most meaningful layout possible, and they shouldn't overlap.
        ...
        '''''
        Here is the prompt from the user to either create or update the existing json scene:

        {update_prompt}

        '''''
        Required Output: You must format your output as a JSON dictionary that adheres a given JSON schema instance with the following keys:
        ...
        """
        prompt_from_template = PromptTemplate(template=template, input_variables=["previous_scene_json","update_prompt"])
        llm_chain = LLMChain(prompt=prompt_from_template,llm=self.llm, verbose=True)
        response = llm_chain.run(previous_scene_json=str(self.previous_scene_json), update_prompt=self.update_prompt)
        print(response)
        try:
